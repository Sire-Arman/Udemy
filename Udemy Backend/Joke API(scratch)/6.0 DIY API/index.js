...
app.put("/replace/:id", (req,res) =>{
  try {
    ...
    jokes[ind].jokeText = joke.jokeText;
    jokes[ind].jokeType = joke.jokeType;
...
app.patch("/patch/:id", (req,res) =>{
  try {
    ...
    var a;
    const ind = jokes.findIndex((j) => j.id === id);
    if(joke.jokeText) { a = jokes[ind].jokeText; jokes[ind].jokeText = joke.jokeText;}
    if(joke.jokeType) jokes[ind].jokeType = joke.jokeType;
    console.log(jokes[ind]);
    res.send("change from " +a+" to "+jokes[ind].jokeText); 
  } catch (error) {
    res.status(404).send(error.message)
  }
})
...
//8. DELETE All jokes
app.delete("/deleted/all", (req,res) =>{
  try {
    jokes = [];
    res.send("deleted the database"); 
  } catch (error) {
    res.status(404).send(error.message)
  }
})
